package com.aurora.ai.neuralnetwork.prune;
...
import com.aurora.ai.neuralnetwork.feedforward.train.backpropagation.Backpropagation;
...
public class Prune {
	 ...
	 * @param ideal
	 */
	public Prune(final NeuralNetwork network, final double train[][],
	 ...
	 */
	protected NeuralNetwork clipHiddenNeuron(final int neuron) {
		final NeuralNetwork result = (NeuralNetwork) this.currentNetwork
				.clone();
		final Collection<NeuralLayer> c = result.getHiddenLayers();
		final Object layers[] = c.toArray();
		((NeuralLayer) layers[0]).prune(neuron);
		return result;
	}
	 ...
	 */
	protected double determineError(final NeuralNetwork network) {
	...
	protected boolean findNeuron() {
		...
		for (int i = 0; i < this.currentNetwork.getHiddenLayerCount(); i++) {
			final NeuralNetwork trial = this.clipHiddenNeuron(i);
	...
	protected int getHiddenCount() {
		final Collection<NeuralLayer> c = this.currentNetwork
				.getHiddenLayers();
		final Object layers[] = c.toArray();
		return ((NeuralLayer) layers[0]).getNeuronCount();
	}
	...
	protected void increment() {
		...
		if (doit) {
			...
			this.currentNetwork.addLayer(new NeuralLayer(
					this.train[0].length));
			this.currentNetwork.addLayer(new NeuralLayer(
					this.hiddenNeuronCount));
			this.currentNetwork.addLayer(new NeuralLayer(
	...
	public void startIncremental() {
		...
		this.currentNetwork = new NeuralNetwork();
		this.currentNetwork
				.addLayer(new NeuralLayer(this.train[0].length));
		this.currentNetwork.addLayer(new NeuralLayer(
				this.hiddenNeuronCount));
		this.currentNetwork
				.addLayer(new NeuralLayer(this.ideal[0].length));
		this.currentNetwork.reset();
